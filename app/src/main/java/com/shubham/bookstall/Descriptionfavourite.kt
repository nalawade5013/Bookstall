...
class Descriptionfavourite : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_descriptionfavourite)
        ...
        supportActionBar?.title="Deatails"
        ...
        if(Connectionmanager().chekconnection(this)){
            val jsonobjectrequest =object : JsonObjectRequest(
                Request.Method.POST,url,jsonparameter,
                Response.Listener {
                ...
                try{
                    ...
                    if(succes){
                        ...
                        val checkfav= Descriptionactivity.DBasync(
                            applicationContext,
                            bookenitites,
                            1
                        ).execute()
                        ...
                        button.setOnClickListener {
                            if(!Descriptionactivity.DBasync(applicationContext, bookenitites, 1).execute().get()){

                                val async=
                                    Descriptionactivity.DBasync(applicationContext, bookenitites, 2)
                                        .execute()
                                ...
                                val async=
                                    Descriptionactivity.DBasync(applicationContext, bookenitites, 3)
                                        .execute()
                                val result=async.get()
                                if(result){
